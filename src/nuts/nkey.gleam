...
pub fn from_seed(seed: String) -> Result(KeyPair, NkeyError) {
  use <- bool.guard(
  ...
  case bin {
    <<raw:bytes-size(size - 2), crc_checksum:16-little>> -> {
      use <- bool.guard(
        when: crc_checksum != crc(raw),
        return: Error(InvalidChecksum),
      )

      case raw {
        <<18:5, p2:5, _:6, rest:bytes>> -> {
          generate_key(rest)
          |> result.replace_error(NkeyError("failed to generate key pair"))
          |> result.map(fn(pair) {
            let #(public, private) = pair

            KeyPair(public:, private:, kind: p2)
          })
        }
        _ -> Error(NkeyError("invalid NKEY"))
      }
    }
    _ -> Error(NkeyError("unreachable"))
  }
}
...
pub fn public(pair: KeyPair) {
  ...
  encode32(<<prefixed:bits, checksum:16-little>>)
}

pub fn sign(key_pair: KeyPair, data: BitArray) -> BitArray {
  ...
  case input {
    ...
    <<byte, rest:bits>> -> {
      let key =
        ...
        |> int.bitwise_exclusive_or(byte)
        |> int.bitwise_and(0x00ff)

      let crc =
        int.bitwise_shift_left(crc, 8)
        |> int.bitwise_exclusive_or(crc_table(key))
